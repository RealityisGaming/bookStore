...
mulitple_book_schema = BookSchema(many=True)
...
def add_book():
    ...
    if book:
        return jsonify("Can't use that title my guy")

    new_book = Book(title, author, review, genre, price)
    db.session.add(new_book)
    db.session.commit()

    return jsonify("You did it! Very unique")
...
def delete_book(id):
    ...
    return jsonify('The book {book.title} has been deleted.')
@app.route("/book/get", methods=["GET"])
def get_books():
    books = db.session.query(Book).all()
    return jsonify(mulitple_book_schema.dump(books))
